						...
						<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
							<header class="entry-header">
								...
								<div class="entry-meta">
									<?php
										...
										printf( __( 'Published', 'cyberchimpspro' ) . ' <span class="entry-date"><time class="entry-date" datetime="%1$s">%2$s</time></span> - ' . __( 'size', 'cyberchimpspro' ) . ': <a href="%3$s" title="Link to full-size image">%4$s &times; %5$s</a> ' . __( 'in', 'cyberchimpspro' ) . ' <a href="%6$s" title="Return to %7$s" rel="gallery">%7$s</a>',
									...
									?>
									<?php edit_post_link( __( 'Edit', 'cyberchimpspro' ), '<span class="sep"> | </span> <span class="edit-link">', '</span>' ); ?>
								</div><!-- .entry-meta -->
			
								<nav id="image-navigation">
									<span class="previous-image"><?php previous_image_link( false, '&larr; ' . __( 'Previous', 'cyberchimpspro' ) ); ?></span>
									<span class="next-image"><?php next_image_link( false, __( 'Next', 'cyberchimpspro' ) . ' &rarr;' ); ?></span>
							...
							<footer class="entry-meta">
								<?php if ( comments_open() && pings_open() ) : // Comments and trackbacks open ?>
									<?php printf( '<a class="comment-link" href="#respond" title="Post a comment">' . __( 'Post a comment', 'cyberchimpspro' ) . '</a> ' . __( 'or leave a trackback', 'cyberchimpspro' ) . ': <a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'cyberchimpspro' ) . '</a>.', get_trackback_url() ); ?>
								<?php elseif ( ! comments_open() && pings_open() ) : // Only trackbacks open ?>
									<?php printf( __( 'Comments are closed, but you can leave a trackback: ', 'cyberchimpspro' ) . '<a class="trackback-link" href="%s" title="Trackback URL for your post" rel="trackback">' . __( 'Trackback URL', 'cyberchimpspro' ) . '</a>.', get_trackback_url() ); ?>
								<?php elseif ( comments_open() && ! pings_open() ) : // Only comments open ?>
									<?php printf( __( 'Trackbacks are closed, but you can %s', 'cyberchimpspro' ),
													'<a class="comment-link" href="#respond" title="Post a comment">' . __( 'post a comment', 'cyberchimpspro' ) . '</a>.' ); ?>
								<?php elseif ( ! comments_open() && ! pings_open() ) : // Comments and trackbacks closed ?>
									<?php _e( 'Both comments and trackbacks are currently closed.', 'cyberchimpspro' ); ?>
								<?php endif; ?>
